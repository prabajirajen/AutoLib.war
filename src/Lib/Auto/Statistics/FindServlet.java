 ...
 if(value.equals("Sup"))
  {
		...
		SQLStr="select * from sup_pub_mas where 2>1 and sp_code<>1 and SP_name like  '" + nm + "%' and sp_type='SUPPLIER' "+strsql+" order by SP_name ";
        st=con.createStatement();
        rs = st.executeQuery(SQLStr);
...
if(value.equals("Pub"))
			...
			SQLStr="select * from sup_pub_mas where 2>1 and sp_code<>1 and SP_name like '" + nm + "%'  and sp_type='PUBLISHER' "+strsql+" order by SP_name ";
  st=con.createStatement();
  rs = st.executeQuery(SQLStr);
    while(rs.next()){
