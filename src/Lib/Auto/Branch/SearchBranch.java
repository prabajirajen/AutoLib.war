...
public class SearchBranch extends HttpServlet implements Serializable {
...
singlecodecheck code=new singlecodecheck();
	...
	public void performTask(
		...
		try {
			...
			SQLString ="select * from branch_mas where 2>1 and Branch_name like '" + nm + "%' order by Branch_code";
  ...
  rs = st.executeQuery(SQLString);
    while(rs.next()){
		...
		ob.setAl(ser);
	 getServletConfig().getServletContext().getRequestDispatcher("/Branch/search.jsp?check=ok&nam="+nm+"").forward(request, response);
